...
echo "${white}---> I AM Mass_Print_Artists.sh"
echo "${white}---> Rename the tract artist id3 tags in the directory"
mkdir -p ../_Mass_Print_Artists
read -p "Put your folder/files in _Mass_Print_Artists/folder_With tracks
${orange}then Press Enter${white} :" enter

for directory in "$parentdir"/_Mass_Print_Artists/*/
do
cd "$directory"
...
read -p "${white}---> Enter the Name of the artist             :${orange}" artist
...
ffmpeg -i "$LineFileListe" -c copy -metadata artist="$artist" ../_Processed/"$ParentDir2Album"/"$filename"
echo $filename $purple $artist
done
#
