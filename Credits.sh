...
Track_position=$(echo "$disk"-"$track"|sed 's/CD//g')
else
Track_position=$(echo "$TheLineInfoOnTrack" |awk -F'trackPos_2RCje">' '{print $2}'|awk -F'<' '{print "1-"$1}'|awk NF| sed 's/^0*//')
fi
...
TRACTITLE_ON_DOSCOGS=$(cat diskinfo/SongInfo/"$DiscNumber"-"$tractNumber"_Song_info.txt |awk 'NR == 2')
...
((SELECTION--))
echo "${bg_blue}${white}########################################################################################################################################################
#${bg_red}${white}---> Source File Trac Name = ${orange}$TrackTitle${white} /// Discogs Track Name = ${orange}$TRACTITLE_ON_DOSCOGS${white}  ${orange}DiscNumber:${white}$DiscNumber ${orange}TractNumber:${white}$tractNumber <---${reset}
${white}########################################################################################################################################################${reset}"
...
cat diskinfo/SongInfo/"$DiscNumber"-"$tractNumber"_Song_info.txt | awk '(NR>2)' | awk -F'|' '{print $2, $1}' OFS='|' > diskinfo/SongInfo/"$DiskSongNumber"_taxo_track_info.txt
IFS=$'\n'       # Processing line
set -f          # disable globbing
index=0
