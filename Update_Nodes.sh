...
echo "${bg_blue}${white}---> JE SUIS                                          : Update_Nodes.sh  <---${reset}"
mkdir -p "$dir"/tmp

if [ -f TheRightPath ]
then
TheRightPath=$(cat TheRightPath)
echo "${white}${bg_red}---> The working directory (\$TheRightPath) is  : ${white}$TheRightPath <---${reset}"

read -p "Choices
1 = Keep Actual Dir structure   :
2 = Change working location     : " Choices
echo "$Choices" > tmp/directory_choice
directory_choice=$(cat tmp/directory_choice)


if [ "$directory_choice" == 1 ]
then
echo "${white}---> Using as working directory                 : ${white}$TheRightPath <---${reset}"
Base_directory=$(cat TheRightPath)
elif [ "$directory_choice" == 2 ]
then
read -p "${white}${bg_red}---> Enter the new working directory path ${reset} : " TheRightPath
echo "$TheRightPath" > TheRightPath
else
exit 1
fi
else
read -p "${white}${bg_red}Enter working directory path ${reset} :" TheRightPath
echo "$TheRightPath" > TheRightPath
fi
...
if [ -f "$dir"/tmp/temp_update_lines ]
then
rm "$dir"/tmp/temp_update_lines
fi
#Path2album="/Volumes/MUSIC_OUT/Test"
#TheRightPath="/Volumes/MUSIC_OUT/Test"
find "$TheRightPath" -name *_NodeAudio.csv | sed 's/\/\//\//g' > tmp/NodeAudioInfoTMP

cat tmp/NodeAudioInfoTMP | awk -F'_album_info' '{print $1}'|awk '!seen[$0]++' > tmp/directories.txt

IFS=$'\n'
set -f
for Path2album in $(cat tmp/directories.txt)
do

cd "$Path2album"
mkdir -p _zip_archives
find _album_info -name *_NodeAudio.csv | sed 's/\/\//\//g' > "$dir"/tmp/the_current_disc_csv_list.txt
ziplist=$(cat "$dir"/tmp/the_current_disc_csv_list.txt)
FileDate=$(echo $(date +%Y_%m_%d_%Hh%Mm%Ss) | tr "/" "_")
zip _zip_archives/_NodeAudio_"$FileDate".zip $ziplist
...
find "$Path2album" -name *_NodeAudio.csv | sed 's/\/\//\//g' > "$dir"/tmp/NodeAudioInfoTMP
IFS=$'\n'
set -f
for theline in $(cat "$dir"/tmp/NodeAudioInfoTMP)
do
#echo $purple "$theline >>><<<>>><<<>>><<<<>><<<<>><<<>>><<<>>"
begining=$(awk -F'|' '
NR==1 {
...
echo "SupportTypeRelease=\$(cat \"\$Path2album\"/_album_info/CSVs/info_support.csv| awk  'NR == 2')" >> "$dir"/tmp/tmp_Bash
...
if [ -f "$dir"/tmp/releaseinfo.tmp ]
then
rm "$dir"/tmp/releaseinfo.tmp
fi
...
echo "$Supports|$TypeRelesae" >> "$dir"/tmp/releaseinfo.tmp
done
else
...
TypeRelease=$(cat "$dir"/tmp/releaseinfo.tmp |awk -F'|' '{print $2}'|tr '\n' '@'|sed 's/.$//'|tr '@' '\n' |awk '!seen[$0]++'|tr '\n' '@')
support=$(cat "$dir"/tmp/releaseinfo.tmp |awk -F'|' '{print $1}'|awk '!seen[$0]++'|tr '\n' '@')

echo "Support_Type|ReleaseType
$support|$TypeRelease" > "$dir"/tmp/info_support.csv
cat "$dir"/tmp/info_support.csv | sed 's/\&quot;/\"/g' > "$Path2album"/_album_info/CSVs/info_support.csv
...
if [ -f "$dir"/tmp/Main_Artist_AlbumTMP ]
then
rm "$dir"/tmp/Main_Artist_AlbumTMP
fi
if [ -f "$dir"/tmp/artist_album_separator ]
then
rm "$dir"/tmp/artist_album_separator
fi
...
#echo "$artist_line"  >> "$dir"/tmp/artist_album_separator


# Artist-address|ArtistID|Artist_TID|Artist
echo "$MainArtistUrl|$MainArtistID|$MainArtistTID|$MainArtistName|$SeparatorPostSpan" | sed 's/\ –\ <\!-- -->//g'>> "$dir"/tmp/Main_Artist_AlbumTMP
done

echo "Artist-address|ArtistID|Artist_TID|Artist|sufix-title" > "$Path2album"/_album_info/_info_Artist/Main_Artist_Album.csv
cat "$dir"/tmp/Main_Artist_AlbumTMP | sed "s/&amp;/\&/g" | sed "s/&#39;/'/g" | sed 's/&#34;/"/g' >> "$Path2album"/_album_info/_info_Artist/Main_Artist_Album.csv
fi
...
echo "$begining|$ALBUMARTIST|$Segment1|$Label1|$LabeCatalogNumber1|$Label2|$LabeCatalogNumber2|$Label3|$LabeCatalogNumber3|$Segment2|$YearMaster|$SupportTypeRelease|$sufix_artist_group" >> "$dir"/tmp/temp_update_lines
echo "DiscNumber|traxNumber|Artist|Album_Title|CoverFID|TrackTitle|ALBUMARTIST|DISCTOTAL|TRACKTOTAL|Duration|Audio|Label1|LabeCatalogNumber1|Label2|LabeCatalogNumber2|Label3|LabeCatalogNumber3|YEAR|Date|fileNoExt|Path2album|extension|FileSize|Genre|AlbumADDRESS|Album_TID|TheFileSoundID_fid|ID_DISCOGS|ImagesFID|NodeID|UUID|Artist_TID|Album_TID|AlbumURL|extension|TrackURL|FileOutNoExt|ISRC|compilation|compilationUUID|1_pers|1_info|2_pers|2_info|3_pers|3_info|4_pers|4_info|5_pers|5_info|6_pers|6_info|7_pers|7_info|8_pers|8_info|9_pers|9_info|10_pers|10_info|Job_10|ArtistTID_10|Job_9|ArtistTID_9|Job_8|ArtistTID_8|Job_7|ArtistTID_7|Job_6|ArtistTID_6|Job_5|ArtistTID_5|Job_4|ArtistTID_4|Job_3|ArtistTID_3|Job_2|ArtistTID_2|Job_1|ArtistTID_1|releasenoteslines|BarCode|MainArtistAddress|YearMaster|Support_Type|ReleaseType|sufix_artist_group" > tmp.txt
echo "$begining|$ALBUMARTIST|$Segment1|$Label1|$LabeCatalogNumber1|$Label2|$LabeCatalogNumber2|$Label3|$LabeCatalogNumber3|$Segment2|$YearMaster|$SupportTypeRelease|$sufix_artist_group" >> tmp.txt
mv tmp.txt "$theline"
#read -p "waite here" wait
done
...
cd "$dir"
echo "$purple$Path2album"
echo "$white$(pwd)"
done
FileDate=$(echo $(date +%Y_%m_%d_%Hh%Mm%Ss) | tr "/" "_")
#TrashDirectory=$(echo "$Album_Title"| tr -d '/:')
echo "DiscNumber|traxNumber|Artist|Album_Title|CoverFID|TrackTitle|ALBUMARTIST|DISCTOTAL|TRACKTOTAL|Duration|Audio|Label1|LabeCatalogNumber1|Label2|LabeCatalogNumber2|Label3|LabeCatalogNumber3|YEAR|Date|fileNoExt|Path2album|extension|FileSize|Genre|AlbumADDRESS|Album_TID|TheFileSoundID_fid|ID_DISCOGS|ImagesFID|NodeID|UUID|Artist_TID|Album_TID|AlbumURL|extension|TrackURL|FileOutNoExt|ISRC|compilation|compilationUUID|1_pers|1_info|2_pers|2_info|3_pers|3_info|4_pers|4_info|5_pers|5_info|6_pers|6_info|7_pers|7_info|8_pers|8_info|9_pers|9_info|10_pers|10_info|Job_10|ArtistTID_10|Job_9|ArtistTID_9|Job_8|ArtistTID_8|Job_7|ArtistTID_7|Job_6|ArtistTID_6|Job_5|ArtistTID_5|Job_4|ArtistTID_4|Job_3|ArtistTID_3|Job_2|ArtistTID_2|Job_1|ArtistTID_1|releasenoteslines|BarCode|MainArtistAddress|YearMaster|Support_Type|ReleaseType|sufix_artist_group" > ../_Batch_Import_"$FileDate".csv
cat "$dir"/tmp/temp_update_lines >> ../_Batch_Import_"$FileDate".csv
