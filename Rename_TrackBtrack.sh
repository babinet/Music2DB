...
echo "${WHITE}---> JE SUIS                    : ${green}Rename_TrackBtrack.sh"
echo "${white}---> Je rename les tags id3 des fichiers audio (titre)"
mkdir -p ../_Rename_Track_Title ../_Processed

read -p "put your file in ../_Rename_Track_Title" renum


find "../_Rename_Track_Title" -name "*.flac" -o -name "*.mp3" -o -name "*.m4a" -o -name "*.ogg" -o -name "*.aif"   | sed 's/\/\//\//g' > tmp/listtprenum.txt
...
mkdir -p ../_Rename_Track_Title/tmp
...
ffprobe "$thefilelist" > ../_Rename_Track_Title/tmp/temp_info.txt 2>&1
#cat ../_Rename_Track_Title/tmp/temp_info.txt
#awk '/track/' ../_Rename_Track_Title/tmp/temp_info.txt
extension="${thefilelist##*.}"
FileOutTMP="${thefilelist##*/}"
FileOut=$(echo "$FileOutTMP"|tr ' ' '_')

title=$(cat ../_Rename_Track_Title/tmp/temp_info.txt|awk '/title/'|awk -F'title           : ' '{print $2}')
echo "${white}---> the curent title is          :${orange} $title"
read -p "${white}---> enter the new track name       :${orange}" newtracname
echo "$purple                                   $newtracknumber"

mkdir -p ../_Processed/"$ParentDir2Album"


ffmpeg -i "$thefilelist" -c copy -metadata title="$newtracname" ../_Processed/"$ParentDir2Album"/"$newtracknumber"_"$FileOutNoExt"."$extension"
