...
echo "${white}---> current track is             : ${red}$OriginalTackNumber"
read -p "${white}---> Enter the new track number   :${orange}" NewtrackNumber
mkdir -p ../_Processed/"$ParentDir2Album"/
ffmpeg -i "$LineFileListe" -c copy -metadata track="$NewtrackNumber" ../_Processed/"$ParentDir2Album"/"$filename"
echo $filename $purple $NewtrackNumber
done
#
